...
{                                                 // TWINT = TWI interrupt flag, when set starts the the operation of the TWI
    ...
    if ((TWSR & 0xF8) == TW_START)                // Checking if start went through
        print_status(TW_START);
}

void i2c_stop(void)
{
    TWCR |= (1 << TWSTO) | (1 << TWINT);  // TWSTO : TWI stop to send at the end of communication
}
...
void start_sensor()
{
    i2c_write(SENSOR_ADDR << 1 | 0);                // First transmission is addr of the sensor
    print_status(i2c_getstatus());
    if (i2c_getstatus() != TW_MT_SLA_ACK)
    {
        uart_printstr("error\n");
        return ;  
    }
